...
fn test_basic_context_switch() {
    ...
    let thread_a: ContextThreadEntry = Box::new(move |tid| {
    ...
    let thread_b: ContextThreadEntry = Box::new(move |tid| {
...
fn test_single_thread_yields() {
    ...
    let thread: ContextThreadEntry = Box::new(move |tid| {
...
fn test_three_threads_round_robin() {
    ...
    for i in 0..3 {
        ...
        let thread: ContextThreadEntry = Box::new(move |tid| {
            ...
            let current_len = order.len(); // CORRECCIÓN: obtener len antes de push
            order.push((tid, current_len));
            let count = order.iter().filter(|(t, _)| *t == tid).count();
            drop(order);
