...
fn test_api_join_waits_for_thread_to_finish() {
    ...
    let worker_tid = my_thread_create(
        ...
        // CORRECCIÓN: La clausura ahora acepta dos argumentos (tid, tickets), que ignoramos.
        Box::new(move |_, _| {
    ...
    let worker_finish_pos = log
        .iter()
        .position(|s| s == "Worker: Trabajo terminado")
        .unwrap();
    let waiter_continue_pos = log
        .iter()
        .position(|s| s == "Waiter: Continuo ejecución")
        .unwrap();
    assert!(
        worker_finish_pos < waiter_continue_pos,
        "¡Join falló! El Waiter no esperó al Worker."
    );
...
fn test_api_mutex_provides_mutual_exclusion() {
    ...
    for i in 0..2 {
        ...
        my_thread_create(
            ...
            Box::new(move |_, _| {
                ...
                match *state_val {
                    0 => {
                        // Estado inicial: pedir el lock.
                        *state_val = 1;
                        println!("  [Hilo {}] Intentando tomar el mutex...", i);
                        return my_mutex_lock(&mutex_clone);
                    }
                    1 => {
                        // Despertado: ya tenemos el lock. Trabajar y ceder.
                        *state_val = 2;
                    ...
                    2 => {
                        // Despertado de nuevo: liberar el lock.
                        *state_val = 3;
    ...
    // El orden puede variar, pero debe ser secuencial.
    // Verificamos que un hilo entra y sale antes de que el otro entre.
    let mut last_thread_id = -1;
    let mut in_critical_section = false;

    for entry in log.iter() {
        let parts: Vec<&str> = entry.split_whitespace().collect();
        let thread_id: i32 = parts[1].parse().unwrap();
        let action = parts[2];

        if action == "ENTRA" {
            assert!(!in_critical_section, "¡Exclusión mutua violada! Otro hilo entró antes de que el anterior saliera.");
            in_critical_section = true;
            last_thread_id = thread_id;
        } else if action == "SALE" {
            assert!(in_critical_section, "Un hilo salió sin haber entrado primero.");
            assert_eq!(thread_id, last_thread_id, "¡Salió el hilo equivocado!");
...
fn test_api_mutex_trylock() {
        ...
        Box::new({
            ...
            move |_, _| {
                ...
                match *state_val {
                    ...
                    1 => {
                        // Ya tenemos el lock, ceder para que el otro pruebe
                        *state_val = 2;
    ...
    my_thread_create(
        ...
        Box::new({
            ...
            move |_, _| {
                ...
                match *state_val {
                    0 => {
                        // Ceder para que Locker actúe primero
                        *state_val = 1;
                        return ThreadSignal::Yield;
                    }
                    1 => {
                        // Probar trylock (debería fallar) y ceder de nuevo
                        *state_val = 2;
                        ...
                        if !my_mutex_trylock(&mutex_b) {
                            ...
                            // Si por error lo toma, lo libera inmediatamente.
                    ...
                    2 => {
                        // Probar trylock de nuevo (debería funcionar)
                        *state_val = 3;
                        ...
                        if my_mutex_trylock(&mutex_b) {
                            ...
                            // Liberar el lock que acabamos de tomar con trylock.
                            my_mutex_unlock(&mutex_b);
                        } else {
    ...
    // Validar el orden de los eventos clave
    let acquired_pos = final_log.iter().position(|s| s == "Locker: Lock adquirido").unwrap();
    let failed_pos = final_log.iter().position(|s| s == "TryLocker: trylock falló").unwrap();
    let released_pos = final_log.iter().position(|s| s == "Locker: Lock liberado").unwrap();
    let success_pos = final_log.iter().position(|s| s == "TryLocker: trylock tuvo éxito").unwrap();

    assert!(acquired_pos < failed_pos, "TryLocker debe fallar DESPUÉS de que Locker adquiera el lock.");
    assert!(failed_pos < released_pos, "Locker debe liberar el lock DESPUÉS de que TryLocker falle.");
    assert!(released_pos < success_pos, "TryLocker debe tener éxito DESPUÉS de que Locker libere el lock.");
